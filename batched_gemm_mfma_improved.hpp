...
// ============================================================================
// 128x256 TILE SIZE VERSION
// Based on improved version with 2x4 warp layout (8 warps, 512 threads)
// ============================================================================
    ...
    dim3 strideA, dim3 strideB, dim3 strideC)
{
    ...
    const int warpIdx = threadIdx.x / 64;
    const int warpCol = warpIdx & 3;              // warpIdx % 4 (0-3)
    const int warpRow = warpIdx >> 2;             // warpIdx / 4 (0-1)
    ...
    // ========== Register Allocation ==========
    bf16x4 a[2], b[4];           // Operands for MFMA (2 A tiles, 4 B tiles)
    floatx16 acc[8] = {0};       // Accumulators: 2x4 layout = 8 tiles
    bf16x8 prefetch[2];          // Prefetch registers for A and B
    ...
    *(bf16x8*)(&Bs[writeIdx][bLdsWriteOffset]) = prefetch[1];
    ...
    for (int tile = 0; tile < numTiles; ++tile) {
        ...
        for (int k_iter = 0; k_iter < K_ITERATIONS; ++k_iter) {
            ...
            // Load A operands from LDS (2 tiles for warpRow)
            a[0] = *(bf16x4*)(&As[readIdx][k_offset + aLdsReadOffset]);
            a[1] = *(bf16x4*)(&As[readIdx][k_offset + 64 * BK_PAD + aLdsReadOffset]);
            
            // Load B operands from LDS (4 tiles for warpCol 0-3)
            b[0] = *(bf16x4*)(&Bs[readIdx][k_offset + bLdsReadOffset]);
            b[1] = *(bf16x4*)(&Bs[readIdx][k_offset + 64 * BK_PAD + bLdsReadOffset]);
            b[2] = *(bf16x4*)(&Bs[readIdx][k_offset + 128 * BK_PAD + bLdsReadOffset]);
            b[3] = *(bf16x4*)(&Bs[readIdx][k_offset + 192 * BK_PAD + bLdsReadOffset]);
            
            // MFMA computations: 2x4 = 8 tiles
            // First row (a[0] with all b tiles)
            acc[0] = __builtin_amdgcn_mfma_f32_32x32x8bf16_1k(a[0], b[0], acc[0], 0, 0, 0);
            acc[1] = __builtin_amdgcn_mfma_f32_32x32x8bf16_1k(a[0], b[1], acc[1], 0, 0, 0);
            ...
            // Second row (a[1] with all b tiles)
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        // Convert and store for all 8 accumulator tiles
        bhalf_t val0 = static_cast<bhalf_t>(acc[0][i]);
        bhalf_t val1 = static_cast<bhalf_t>(acc[1][i]);
        bhalf_t val2 = static_cast<bhalf_t>(acc[2][i]);
        bhalf_t val3 = static_cast<bhalf_t>(acc[3][i]);
        bhalf_t val4 = static_cast<bhalf_t>(acc[4][i]);
        bhalf_t val5 = static_cast<bhalf_t>(acc[5][i]);
        bhalf_t val6 = static_cast<bhalf_t>(acc[6][i]);
        bhalf_t val7 = static_cast<bhalf_t>(acc[7][i]);
        
        // Store in column-major order: 4 tiles per row
        C[rowOffset] = val0;
        C[rowOffset + 64] = val1;
        C[rowOffset + 128] = val2;
        C[rowOffset + 192] = val3;
        C[rowOffset + 64 * N] = val4;
        C[rowOffset + 64 * N + 64] = val5;
        C[rowOffset + 64 * N + 128] = val6;
        C[rowOffset + 64 * N + 192] = val7;
    }
}
